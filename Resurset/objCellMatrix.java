...
public class objCellMatrix {
	...
	private int[][] playerMatrix = new int[8][8]; // Stores which player is in a cell. 0 empty, 1 player one, 2 player
													// two
	private int[][] pieceMatrix = new int[8][8]; // Stores which piece is in a cell. 0 pawn, 1 rock, 2 knight, 3 bishop,
	...
	public boolean checkWinner(int currentPlayer) {
		...
		for (int row = 0; row < 8; row++) {
			...
			for (int column = 0; column < 8; column++) {
				...
				if (playerMatrix[row][column] == checkPlayer && pieceMatrix[row][column] == 5) // If the enemy's king
																								// still remains
				{
					return false;
				}
